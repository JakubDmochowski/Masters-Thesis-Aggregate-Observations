...
train_file = os.getcwd() + "/datasets/monks/monks-1.train"
test_file = os.getcwd() + "/datasets/monks/monks-1.test"
...
def get_testing_data():
    return encode_data(pd.read_csv(test_file, delim_whitespace=True, header=None))
...
class MonksDataGraph(nx.DiGraph):
    def __init__(self):
        super().__init__(self)
    ...
    def get_data(self):
        data = pd.read_csv(train_file, delim_whitespace=True, header=None)
        data = data.drop(columns=[7]) #ignore row ids
