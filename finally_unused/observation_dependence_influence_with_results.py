...
from data.synthetic import generate_data, generate_dependent_observations, aggregate_mean, SyntheticDataGraph
...
for i in range(REPEAT_TIMES):
    ...
    for DIM_INDEX, NUM_DIMENSIONS in enumerate(DIMS):
        ...
        data_graph = SyntheticDataGraph(data_x, data_y, dep_obs_y)
        data_graph.prep()
        ...
        for iterIndex in trange(NUM_ITERS):
            ...
            if not iterIndex % VALIDATE_EVERY_K_ITERATIONS:
                with torch.no_grad():
                    ...
                    dependent_quality_history.append(
                        torch.nn.functional.mse_loss(data_y_qs.reshape(-1), dependent_predictions.reshape(-1)))
                    independent_quality_history.append(
                        torch.nn.functional.mse_loss(data_y_qa.reshape(-1), independent_predictions.reshape(-1)))
