...
from models.XBNet.training_utils import training
...
def run_XBNET(X_train, X_test, y_train, y_test, model, criterion, optimizer, batch_size=16, epochs=100):
    ...
    """
    train_dataload = DataLoader(Data(X_train, y_train), batch_size=batch_size)
    test_dataload = DataLoader(Data(X_test, y_test), batch_size=batch_size)
    acc, lo, val_ac, val_lo = training(model, train_dataload, test_dataload, criterion, optimizer, epochs)
